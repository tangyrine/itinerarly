...
function debugAuthTokens(): string {
  ...
  .then(data => {
    ...
    // Authentication successful - do NOT automatically test token consumption
    // Token consumption should only happen when user explicitly clicks Generate buttons
    if (data && !data.error && data.remainingTokens !== undefined) {
      console.log("✅ Authentication successful! Remaining tokens:", data.remainingTokens);
      console.log("💡 Note: Automatic token consumption test DISABLED to prevent accidental usage");
      console.log("🔧 To manually test token consumption, run: fetch('/api/v1/tokens/consume', {method: 'POST', credentials: 'include'})");
    } else {
      console.log("❌ Authentication failed or no token data available");
    }
  })
  .catch(error => {
