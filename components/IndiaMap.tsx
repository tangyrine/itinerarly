...
export default function IndiaMap({ type }: IndiaMapProps) {
  ...
  const handleStateClick = async (geo: any, e: React.MouseEvent<SVGPathElement>) => {
    ...
    const res = await axios.post("/api/stateDetails", { placeName: stateName });
  ...
  const handleShowDetails = async () => {
    if (hoveredPlace) {
      if (!markerDetailsMap[hoveredPlace]) await fetchDetailsForMarker(hoveredPlace);
  ...
  return (
    <div className="w-full h-full bg-blue-500 relative overflow-hidden">
      <div className="absolute top-5 right-5 z-10 flex flex-col gap-2">
        <button onClick={() => setPosition((pos) => ({ ...pos, zoom: pos.zoom * 1.5 }))} className="p-2 bg-white cursor-pointer rounded-full shadow-lg hover:bg-gray-100 transition-colors">
          <Plus className="w-6 h-6" />
        </button>
        <button onClick={() => setPosition((pos) => ({ ...pos, zoom: pos.zoom / 1.5 }))} className="p-2 bg-white cursor-pointer rounded-full shadow-lg hover:bg-gray-100 transition-colors">
          <Minus className="w-6 h-6" />
        </button>
        <button onClick={() => setPosition({ coordinates: [82, 22], zoom: 1 })} className="p-2 bg-white rounded-full cursor-pointer shadow-lg hover:bg-gray-100 transition-colors">
      ...
      {isMapLoading ? (
        ...
        <ComposableMap projection="geoMercator" projectionConfig={{ scale: 800, center: [82, 22] }} style={{ width: "100%", height: "100%" }}>
          <ZoomableGroup zoom={position.zoom} center={position.coordinates} onMoveEnd={({ coordinates, zoom }: { coordinates: [number, number]; zoom: number }) => setPosition({ coordinates, zoom })} maxZoom={4} minZoom={1}>
                ...
                geographies.map((geo) => {
                  ...
                  return (
                    <Geography
                      ...
                      onClick={(e: React.MouseEvent<SVGPathElement, MouseEvent>) => handleStateClick(geo, e)}
                      stroke="#FFFFFF"
                      strokeWidth={0.5}
                      style={{
                        default: {
                          fill: isSelected ? "#4299E1" : isHovered ? "#FFA500" : "#D6D6DA",
                          outline: "none",
                        },
                        hover: {
            ...
            {highlightedPlaces.map((place: any) => (
              <Marker key={place.name} coordinates={[place.coordinates[0], place.coordinates[1]]}>
                <g
                ...
                >
                  <circle
                    ...
                    fill={selectedPlace === place.name ? "#FF8C00" : hoveredPlace === place.name ? "#FFA500" : "#F53"}
                    stroke="#fff"
                    strokeWidth={2}
      ...
      {hoveredPlace && !showPlaceDetails && (
        <div
          ...
          style={{ left: mousePosition.x + 10, top: mousePosition.y + 10, maxWidth: "300px" }}
        ...
        >
          ...
          {loadingMarker === hoveredPlace ? (
            ...
            <button className="text-blue-400 text-center hover:underline" onClick={handleShowDetails}>
              Show details
            </button>
          )}
        </div>
      ...
      {hoveredState && (
        <div
          ...
          style={{ left: stateMousePosition.x + 10, top: stateMousePosition.y + 10, pointerEvents: "none" }}
        >
          {hoveredState}
        </div>
      )}
      ...
      {showPlaceDetails && selectedPlace && (() => {
        const selected = highlightedPlaces.find((p: any) => p.name === selectedPlace);
        return (
          <PlaceDetails
            place={selectedPlace}
